...
fn is_broken_shim(path: &Path, home: &Option<PathBuf>) -> bool {
    ...
    if !path.starts_with(home.join(".local/share/mise/shims")) {
        return false;
    }
    #[cfg(unix)]
    {
...
fn probe_mise_managed(entry: &InstallCatalogEntry, opts: &DetectOptions) -> Option<Vec<Detection>> {
    let exec_opts = ExecuteOpts {
        timeout: Some(Duration::from_secs(2)),
        output_limit: Some(8 * 1024),
        ..Default::default()
...
fn probe_homebrew(
    ...
    // brew prints: "<package> 1.2.3 1.2.2"
        ...
        .and_then(|o| {
            if o.success {
                ...
                if prefix.is_empty() {
                    ...
                    // npm global bin path is <prefix>/bin/<exe>.
                    Some(
                        PathBuf::from(prefix)
                            .join("bin")
                            .join(entry.executables.first().map_or(package, String::as_str)),
...
fn extract_npm_version(json: &str, package: &str) -> Option<String> {
    if let Ok(val) = serde_json::from_str::<serde_json::Value>(json) {
        ...
        // npm sometimes nests a renamed package; accept the first dependency.
        if let Some(v) = val
            .get("dependencies")
...
fn probe_cargo(
    ...
    // cargo install --list prints "package v1.2.3:" headers with indented
    // binary lines under each.
        ...
        if !line.starts_with(' ') && !line.starts_with('\t') {
            ...
            if pkg == package {
                let exe_candidate = trimmed
                    ...
                    .trim_end_matches(':')
                    .to_owned();
                // `binary "exe":`, `exe:`, and bare lines all reduce to the
                // catalog's first executable when the token is unusable.
                let exe_name = if exe_candidate.is_empty() || exe_candidate == "binary" {
                    entry
                        .executables
                        .first()
...
fn probe_app_bundle_version(bundle_path: &Path) -> Option<String> {
    // Direct plist read: <key>CFBundleShortVersionString</key> followed by
    // <string>VERSION</string>. No `defaults read`; keeps the probe shell-free.
...
fn probe_bundle_id(bundle_id: &str, opts: &DetectOptions) -> Option<PathBuf> {
    // `mdfind` returns app paths for the bundle id; absent mdfind means no evidence.
    let exec_opts = ExecuteOpts {
        timeout: Some(opts.probe_timeout),
        output_limit: Some(8 * 1024),
        ..Default::default()
