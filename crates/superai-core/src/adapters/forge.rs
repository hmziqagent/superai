//! Forge adapter — relocated config via `FORGE_CONFIG` with TOML/YAML/JSON.
//!
//! Research source: `docs/harness-configs/forge.md` (last verified 2026-08-25).
//! Executable `forge`, config root `~/.forge` or `$FORGE_CONFIG`, primary
//! writable surface `.forge.toml` (TOML), isolation `relocated-root`.
...
///
/// Isolation is `relocated-root` via `FORGE_CONFIG`. The wrapper sets
/// `FORGE_CONFIG` to the instance `config_root` and execs `forge`.
#[derive(Debug, Clone)]
...
impl ForgeAdapter {
    ...
    /// Try to locate the `forge` binary via `PATH`.
    ...
    /// Probe `forge --version` with a timeout, returning the parsed version string if successful.
    ...
    /// Parse version output like `forge 0.3.0` into `0.3.0`.
    #[expect(
...
mod tests {
    ...
    fn parse_version_output_cases() {
        ...
        for (input, expected) in cases {
            let got = ForgeAdapter::parse_version_output(input);
    ...
    fn plan_wrapper_sets_forge_config() {
        let a = adapter();
        let inst = sample_instance_with_root("/tmp/.forge-work");
        let plan = a.plan_wrapper(&inst).unwrap();
        assert!(
            plan.env_vars
                .iter()
                .any(|(k, v)| k == CONFIG_ENV_VAR && v == "/tmp/.forge-work")
        );
    ...
    fn plan_wrapper_quoting_with_spaces() {
        let a = adapter();
        let inst = sample_instance_with_root("/tmp/my forge work");
        let plan = a.plan_wrapper(&inst).unwrap();
        let env_val = plan
            .env_vars
            .iter()
            ...
            .map(|(_, v)| v.as_str())
            .unwrap();
        assert_eq!(env_val, "/tmp/my forge work");
    ...
    fn plan_wrapper_rejects_mismatched_harness() {
        let a = adapter();
        let mut inst = sample_instance_with_root("/tmp/.forge-work");
    ...
    fn wrapper_env_var_isolation_is_relocated_root() {
        ...
        let inst = sample_instance_with_root("/home/user/.forge-isolated");
        let plan = a.plan_wrapper(&inst).unwrap();
        assert!(!plan.env_vars.is_empty());
        let (key, val) = &plan.env_vars[0];
        assert_eq!(key, CONFIG_ENV_VAR);
        assert_eq!(val, "/home/user/.forge-isolated");
    }
