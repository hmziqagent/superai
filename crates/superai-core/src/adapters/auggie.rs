...
mod tests {
    ...
    fn fixture_populated_loads_with_documented_keys() {
        let path = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("fixtures/auggie")
            .join("settings.populated.json");
        assert!(path.exists(), "fixture missing: {}", path.display());
        let value = superai_config::json::load(&path).unwrap();
        assert!(
            value.contains_key("autoUpdate"),
            "fixture must document `autoUpdate`"
        );
        let report = crate::verification::fixture_report(path.parent().unwrap());
        assert!(report.validity_pass, "auggie corpus validity");
        assert!(report.secret_free_pass, "auggie corpus secret-free");
    }
}
