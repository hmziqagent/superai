//! Hermes adapter — relocated-root/profile via `HERMES_HOME`+`--profile` with YAML `config.yaml`
//! and env `profiles`/`skills`/`MCP`.
//!
//! Research source: `docs/harness-configs/hermes-agent.md` (last verified 2026-08-25).
//! Executable `hermes`, config root `~/.hermes` or `$HERMES_HOME`,
...
/// Harness identifier for Hermes Agent.
pub const HARNESS_ID_STR: &str = "hermes-agent";

/// Human display name.
pub const DISPLAY_NAME: &str = "Hermes Agent";

/// Primary executable name.
pub const EXECUTABLE: &str = "hermes";

/// Environment variable that relocates the config root.
pub const CONFIG_ENV_VAR: &str = "HERMES_HOME";

/// Profile selector (alternative to --profile flag).
pub const PROFILE_ENV_VAR: &str = "HERMES_PROFILE";

/// Default config root when `HERMES_HOME` is unset.
pub const DEFAULT_CONFIG_ROOT_FALLBACK: &str = "~/.hermes";

/// Research document link.
pub const RESEARCH_DOC: &str = "docs/harness-configs/hermes-agent.md";
...
/// Concrete adapter for Hermes Agent.
///
/// Isolation is `relocated-root` via `HERMES_HOME` (plus `--profile`/`HERMES_PROFILE` for sub-profiles).
/// The wrapper sets `HERMES_HOME` to the instance `config_root` and execs `hermes`.
#[derive(Debug, Clone)]
...
impl HermesAdapter {
    ...
    fn collect_config_evidence(&self, evidence: &mut Vec<String>) {
        match Self::default_config_root() {
            Some(root) => {
                if root.exists() {
                    ...
                    let env = Self::env_path_for_root(&root);
                    if env.exists() {
                        evidence.push(format!(".env present at {}", env.display()));
                    }
                    ...
                    if profiles.exists() {
                        evidence.push(format!("profiles dir present at {}", profiles.display()));
                    }
                } else {
            ...
            && !dir.trim().is_empty()
        {
            ...
            evidence.push(format!("{CONFIG_ENV_VAR} not set, using ~/.hermes"));
        }
        if let Ok(profile) = std::env::var(PROFILE_ENV_VAR)
            && !profile.trim().is_empty()
        {
            evidence.push(format!("{PROFILE_ENV_VAR} set to {profile}"));
        }
        if Path::new("config.yaml").exists() || Path::new(".hermes/config.yaml").exists() {
            evidence.push("project hermes config found".to_owned());
        }
    }
}
...
impl Adapter for HermesAdapter {
    ...
    fn version_resolution(&self) -> VersionResolution {
        ...
        if let Some(v) = detection.version {
            ...
            notes.push(format!("detected hermes-agent version {v}"));
    ...
    fn config_surfaces(&self) -> Vec<ConfigSurface> {
        ...
        let mcp_resolver = PathResolver::new(
            Some("$HERMES_HOME/config.yaml (mcp_servers)"),
            Some("$HERMES_HOME/config.yaml (mcp_servers)"),
            Some("%HERMES_HOME%\\config.yaml (mcp_servers)"),
        ...
        mcp_surface.precedence = 9;
        mcp_surface.owned_selectors = vec!["mcp_servers".to_owned()];
        mcp_surface.backup_required = true;
        surfaces.push(mcp_surface);

        let skills_resolver = PathResolver::new(
            Some("$HERMES_HOME/skills/<name>/SKILL.md"),
            Some("$HERMES_HOME/skills/<name>/SKILL.md"),
            Some("%HERMES_HOME%\\skills\\<name>\\SKILL.md"),
        ...
        let cron_resolver = PathResolver::new(
            Some("$HERMES_HOME/cron/jobs.yaml"),
            Some("$HERMES_HOME/cron/jobs.yaml"),
            Some("%HERMES_HOME%\\cron\\jobs.yaml"),
            "~/.hermes/cron/jobs.yaml",
        );
        let mut cron = ConfigSurface::new(
            "cron/jobs.yaml",
            cron_resolver,
            DocumentKind::Yaml,
            ConfigScope::User,
        ...
        cron.precedence = 6;
        cron.backup_required = false;
        surfaces.push(cron);

        let memories_resolver = PathResolver::new(
            Some("$HERMES_HOME/memories/MEMORY.md"),
            Some("$HERMES_HOME/memories/MEMORY.md"),
            Some("%HERMES_HOME%\\memories\\MEMORY.md"),
    ...
    fn plan_mirror_exclusions(&self) -> Vec<String> {
        vec![
            ...
            "*.tmp".to_owned(),
            ".tmp/*".to_owned(),
            "gateway.pid".to_owned(),
            "gateway.lock".to_owned(),
            "*.lock".to_owned(),
            "state/*".to_owned(),
        ]
    }
    ...
    fn plan_wrapper(&self, instance: &Instance) -> Result<WrapperPlan, CoreError> {
        ...
        instance.validate()?;
        let mut plan =
            WrapperPlan::new("relocated-root via HERMES_HOME (plus --profile for sub-profiles)");
        plan.env_vars
            .push((CONFIG_ENV_VAR.to_owned(), instance.config_root.to_string()));
        plan.description = format!(
            " Wrapper sets {}={} and execs `{}` (use --profile {} for sub-profile)",
            CONFIG_ENV_VAR, instance.config_root, EXECUTABLE, instance.name
        );
        Ok(plan)
    }
    ...
    fn scan_candidates(&self) -> Vec<String> {
        vec![
            "~/.hermes".to_owned(),
            "~/.hermes/profiles/<name>".to_owned(),
            "$HERMES_HOME".to_owned(),
            "~/.hermes-instances/<name>".to_owned(),
            "config.yaml (profile)".to_owned(),
        ]
    }
    ...
    fn validate_instance(&self, instance: &Instance) -> Result<(), CoreError> {
        ...
        match instance.isolation {
            ...
            other => Err(CoreError::Validation {
                ...
                reason: format!("hermes-agent requires isolation relocated_root, got {other}"),
            }),
        }
    }
...
mod tests {
    ...
    use super::{
        CONFIG_ENV_VAR, DISPLAY_NAME, EXECUTABLE, HARNESS_ID_STR, HermesAdapter, OWNED_SELECTORS,
        RESEARCH_DOC,
    };
    ...
    fn adapter() -> HermesAdapter {
        HermesAdapter::new().unwrap()
    }

    fn sample_instance_with_root(root: &str) -> Instance {
        Instance {
            id: InstanceId::new("test-hermes-1").unwrap(),
    ...
    fn parse_version_output_cases() {
        let cases = vec![
            ("hermes 1.2.3", Some("1.2.3")),
            ("hermes 0.1.0-beta", Some("0.1.0-beta")),
            ("v1.0.0", Some("1.0.0")),
        ...
        for (input, expected) in cases {
            let got = HermesAdapter::parse_version_output(input);
    ...
    fn config_surfaces_include_writable_primary() {
        ...
        assert_eq!(primary.kind, DocumentKind::Yaml);
        ...
        for selector in ["model.default", "providers"] {
            assert!(
        ...
        let second = surfaces.iter().find(|s| s.id == ".env").expect(".env");
        assert_eq!(second.kind, DocumentKind::Env);
    }
    ...
    fn plan_wrapper_sets_env() {
        let a = adapter();
        let inst = sample_instance_with_root("/tmp/.hermes-work");
        let plan = a.plan_wrapper(&inst).unwrap();
        assert!(
            plan.env_vars
                .iter()
                .any(|(k, v)| k == CONFIG_ENV_VAR && v == "/tmp/.hermes-work")
        );
    ...
    fn plan_wrapper_quoting_with_spaces() {
        let a = adapter();
        let inst = sample_instance_with_root("/tmp/my hermes work");
        let plan = a.plan_wrapper(&inst).unwrap();
        let env_val = plan
            .env_vars
            .iter()
            ...
            .map(|(_, v)| v.as_str())
            .unwrap();
        assert_eq!(env_val, "/tmp/my hermes work");
    ...
    fn plan_wrapper_rejects_mismatched_harness() {
        let a = adapter();
        let mut inst = sample_instance_with_root("/tmp/.hermes-work");
    ...
    fn scan_candidates_include_default_root() {
        ...
        assert!(
            candidates
                .iter()
                .any(|c| c.contains("HERMES_HOME") || c.contains("hermes") || c.contains('.'))
        );
        assert!(
            candidates
                .iter()
                .any(|c| c.contains(CONFIG_ENV_VAR) || c.contains("HERMES_HOME"))
        );
    }
    ...
    fn path_resolution_resolver_fallbacks() {
        let a = adapter();
        let surfaces = a.config_surfaces();
        let primary = surfaces.iter().find(|s| s.id == "config.yaml").unwrap();
        assert_eq!(primary.path_resolver.fallback, "~/.hermes/config.yaml");
        let resolver = &primary.path_resolver;
        assert!(
            resolver.linux.as_deref().unwrap().contains(CONFIG_ENV_VAR)
                || resolver.linux.as_deref().unwrap().contains("HERMES_HOME")
        );
        assert!(
            resolver.macos.as_deref().unwrap().contains(CONFIG_ENV_VAR)
                || resolver.macos.as_deref().unwrap().contains("HERMES_HOME")
        );
        assert!(
            resolver
                .windows
                ...
                || resolver.windows.as_deref().unwrap().contains("HERMES_HOME")
        );
    }
    ...
    fn fixtures_root() -> PathBuf {
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("fixtures/hermes")
    }
    ...
    fn fixture_minimal_parses() {
        ...
        let map = superai_config::yaml::load(&path).unwrap();
        assert!(map.is_empty() || map.contains_key("model") || map.contains_key("providers"));
    }
    ...
    fn fixture_foreign_survive_because_changing_edit_refuses() {
        ...
        let dir = crate::test_util::temp_dir_unique("hermes");
    ...
    fn secret_redaction_placeholder() {
        ...
        let secret = RedactedString::new("sk-hermes-secret-321");
        let debug = format!("{secret:?}");
        let display = format!("{secret}");
        assert!(!debug.contains("sk-hermes-secret-321"));
        assert!(!display.contains("sk-hermes-secret-321"));
        assert!(debug.contains("[REDACTED]"));
        assert!(display.contains("[REDACTED]"));
        let json = serde_json::to_string(&secret).unwrap();
        assert!(!json.contains("sk-hermes-secret-321"));
        assert!(json.contains("[REDACTED]"));
        assert_eq!(secret.expose_secret(), "sk-hermes-secret-321");
    }
    ...
    fn wrapper_env_var_isolation_is_relocated_root() {
        ...
        let inst = sample_instance_with_root("/home/user/.hermes-isolated");
        let plan = a.plan_wrapper(&inst).unwrap();
        assert!(!plan.env_vars.is_empty());
        let (key, val) = &plan.env_vars[0];
        assert_eq!(key, CONFIG_ENV_VAR);
        assert_eq!(val, "/home/user/.hermes-isolated");
    }
