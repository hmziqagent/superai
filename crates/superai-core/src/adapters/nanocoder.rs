...
pub const OWNED_SELECTORS: &[&str] = &[
    "nanocoder.providers",
    "nanocoder.models",
    "nanocoder.alwaysAllow",
    "nanocoder.disabledTools",
    "mcpServers",
    "preferences",
];
...
impl NanocoderAdapter {
    ...
    fn collect_config_evidence(&self, evidence: &mut Vec<String>) {
        match Self::default_config_root() {
            Some(root) => {
                if root.exists() {
                    ...
                    let prefs = Self::prefs_path_for_root(&root);
                    if prefs.exists() {
                        evidence.push(format!(
                            "nanocoder-preferences.json present at {}",
                            prefs.display()
                        ));
                    }
                    let tools = root.join("tools");
                    if tools.exists() {
                        evidence.push(format!("tools dir present at {}", tools.display()));
                    }
                } else {
            ...
            && !dir.trim().is_empty()
        {
            ...
            evidence.push(format!(
                "{CONFIG_ENV_VAR} not set, using ~/.config/nanocoder"
            ));
        }
        if let Ok(dir) = std::env::var(PROVIDERS_ENV_VAR)
            && !dir.trim().is_empty()
        {
            evidence.push(format!("{PROVIDERS_ENV_VAR} set to {dir}"));
        }
        if let Ok(dir) = std::env::var(MCPSERVERS_ENV_VAR)
            && !dir.trim().is_empty()
        {
        ...
        if project_cfg.exists() {
            evidence.push(format!(
                "project agents.config.json found at {}",
                project_cfg.display()
            ));
        }
        if Path::new(".nanocoder").exists() {
            evidence.push(".nanocoder directory present in cwd".to_owned());
        }
    }
}
...
impl Adapter for NanocoderAdapter {
    ...
    fn config_surfaces(&self) -> Vec<ConfigSurface> {
        ...
        let agents_resolver = PathResolver::new(
            Some("$NANOCODER_CONFIG_DIR/agents.config.json"),
            Some("$NANOCODER_CONFIG_DIR/agents.config.json"),
            Some("%NANOCODER_CONFIG_DIR%\\agents.config.json"),
        ...
        agents_surface.precedence = 10;
        agents_surface.owned_selectors = OWNED_SELECTORS.iter().map(|s| (*s).to_owned()).collect();
        agents_surface.backup_required = true;
        agents_surface.restart_behavior = RestartBehavior::Reload;
        surfaces.push(agents_surface);

        let mcp_resolver = PathResolver::new(
            Some("$NANOCODER_CONFIG_DIR/.mcp.json"),
            Some("$NANOCODER_CONFIG_DIR/.mcp.json"),
            Some("%NANOCODER_CONFIG_DIR%\\.mcp.json"),
            "~/.config/nanocoder/.mcp.json",
        );
        ...
        let prefs_resolver = PathResolver::new(
            Some("$NANOCODER_CONFIG_DIR/nanocoder-preferences.json"),
            Some("$NANOCODER_CONFIG_DIR/nanocoder-preferences.json"),
            Some("%NANOCODER_CONFIG_DIR%\\nanocoder-preferences.json"),
            "~/.config/nanocoder/nanocoder-preferences.json",
        );
        let mut prefs_surface = ConfigSurface::new(
        ...
        prefs_surface.precedence = 5;
        prefs_surface.backup_required = false;
        surfaces.push(prefs_surface);

        let project_agents_resolver = PathResolver::fallback_only("agents.config.json (project)");
        let mut project_agents = ConfigSurface::new(
            "project.agents.config.json",
            project_agents_resolver,
            DocumentKind::Json,
            ConfigScope::ProjectWorkspace,
            SurfaceOwnership::UserEditable,
        );
        project_agents.precedence = 15;
        project_agents.backup_required = false;
        surfaces.push(project_agents);
        ...
        let skills_resolver = PathResolver::new(
            Some("$NANOCODER_CONFIG_DIR/skills/<name>/SKILL.md"),
            Some("$NANOCODER_CONFIG_DIR/skills/<name>/SKILL.md"),
            Some("%NANOCODER_CONFIG_DIR%\\skills\\<name>\\SKILL.md"),
            "~/.config/nanocoder/skills/<name>/SKILL.md",
        );
        ...
        let tools_resolver = PathResolver::new(
            Some("$NANOCODER_CONFIG_DIR/tools/<name>.md"),
            Some("$NANOCODER_CONFIG_DIR/tools/<name>.md"),
            Some("%NANOCODER_CONFIG_DIR%\\tools\\<name>.md"),
            "~/.config/nanocoder/tools/<name>.md",
        );
        ...
        tools.precedence = 6;
        tools.backup_required = false;
        surfaces.push(tools);

        let sessions_resolver = PathResolver::new(
            Some("$NANOCODER_CONFIG_DIR/sessions"),
            Some("$NANOCODER_CONFIG_DIR/sessions"),
            Some("%NANOCODER_CONFIG_DIR%\\sessions"),
    ...
    fn plan_wrapper(&self, instance: &Instance) -> Result<WrapperPlan, CoreError> {
        ...
        let mut plan = WrapperPlan::new(
            "relocated-root via NANOCODER_CONFIG_DIR (explicit override via NANOCODER_PROVIDERS_FILE)",
        );
        plan.env_vars
            .push((CONFIG_ENV_VAR.to_owned(), instance.config_root.to_string()));
        plan.description = format!(
            " Wrapper sets {}={} and execs `{}` (explicit via {}/{})",
            CONFIG_ENV_VAR, instance.config_root, EXECUTABLE, PROVIDERS_ENV_VAR, MCPSERVERS_ENV_VAR
        );
        Ok(plan)
    }
    ...
    fn scan_candidates(&self) -> Vec<String> {
        vec![
            "~/.config/nanocoder".to_owned(),
            "~/.nanocoder".to_owned(),
            "$NANOCODER_CONFIG_DIR".to_owned(),
            "$NANOCODER_PROVIDERS_FILE".to_owned(),
            "agents.config.json".to_owned(),
        ]
    }
    ...
    fn validate_instance(&self, instance: &Instance) -> Result<(), CoreError> {
        ...
        match instance.isolation {
            Isolation::RelocatedRoot | Isolation::ExplicitConfig | Isolation::Unknown => Ok(()),
            other => Err(CoreError::Validation {
                field: "isolation".to_owned(),
                reason: format!(
                    "nanocoder requires isolation relocated_root or explicit_config, got {other}"
                ),
            }),
        }
    }
...
mod tests {
    ...
    fn adapter() -> NanocoderAdapter {
        NanocoderAdapter::new().unwrap()
    }

    fn sample_instance_with_root(root: &str) -> Instance {
        Instance {
            id: InstanceId::new("test-nanocoder-1").unwrap(),
    ...
    fn config_surfaces_include_writable_primary() {
        ...
        let primary = surfaces
            .iter()
            .find(|s| s.id == "agents.config.json")
            .expect("agents.config.json surface must exist");
        ...
        for selector in ["nanocoder.providers", "mcpServers"] {
            assert!(
    ...
    fn plan_mirror_exclusions_cover_history_and_locks() {
        ...
        assert!(!exclusions.contains(&"agents.config.json".to_owned()));
    }
    ...
    fn plan_mirror_includes_settings_and_excludes_sessions() {
        ...
        assert!(!is_excluded("agents.config.json"));
    ...
    fn scan_candidates_include_default_root() {
        ...
        assert!(candidates.iter().any(|c| c.contains("NANOCODER_CONFIG_DIR")
            || c.contains("nanocoder")
            || c.contains('.')));
        assert!(
            candidates
                .iter()
                .any(|c| c.contains(CONFIG_ENV_VAR) || c.contains("NANOCODER_CONFIG_DIR"))
        );
    }
    ...
    fn fixture_foreign_preserves_unknown_keys_on_edit() {
        let path = fixture_path("agents.config.foreign.json");
        assert!(path.exists(), "fixture missing: {}", path.display());
        ...
        let dir = crate::test_util::temp_dir_unique("nanocoder");
        std::fs::create_dir_all(&dir).unwrap();
        let tmp = dir.join("agents.config.foreign.json.copy.json");
    ...
    fn fixture_malformed_fails_to_parse() {
        let path = fixture_path("agents.config.malformed.json");
        assert!(path.exists(), "fixture missing: {}", path.display());
