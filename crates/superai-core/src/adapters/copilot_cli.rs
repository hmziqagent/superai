...
/// Selectors superai owns; everything else round-trips untouched.
...
/// Isolation is `relocated-root` via `COPILOT_HOME`.
#[derive(Debug, Clone)]
...
mod tests {
    ...
    fn plan_wrapper_sets_copilot_home() {
        let tmp_root = crate::test_util::tmp_abs_str(".copilot-work");
        let a = adapter();
    ...
    fn plan_wrapper_quoting_with_spaces() {
        let tmp_root = crate::test_util::tmp_abs_str("my copilot work");
        let a = adapter();
        let inst = sample_instance_with_root(&tmp_root);
        let plan = a.plan_wrapper(&inst).unwrap();
        let env_val = plan
    ...
    fn plan_wrapper_rejects_mismatched_harness() {
        let a = adapter();
        let mut inst = sample_instance_with_root(&crate::test_util::tmp_abs_str(".copilot-work"));
    ...
    fn validate_instance_accepts_relocated_root() {
        let a = adapter();
        let inst = sample_instance_with_root(&crate::test_util::tmp_abs_str(".copilot-work"));
    ...
    fn wrapper_env_var_isolation_is_relocated_root() {
        let tmp_root = crate::test_util::tmp_abs_str("user/.copilot-isolated");
        let a = adapter();
