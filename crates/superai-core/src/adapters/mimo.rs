...
impl Default for MimoAdapter {
    fn default() -> Self {
        #[expect(clippy::unwrap_used, reason = "mimo-code is static valid HarnessId")]
...
mod tests {
    ...
    fn plan_wrapper_sets_env() {
        let tmp_root = crate::test_util::tmp_abs_str(".mimo-work");
        let a = adapter();
    ...
    fn plan_wrapper_quoting_with_spaces() {
        let tmp_root = crate::test_util::tmp_abs_str("my mimo work");
        let a = adapter();
        let inst = sample_instance_with_root(&tmp_root);
        let plan = a.plan_wrapper(&inst).unwrap();
        let env_val = plan
    ...
    fn plan_wrapper_rejects_mismatched_harness() {
        let a = adapter();
        let mut inst = sample_instance_with_root(&crate::test_util::tmp_abs_str(".mimo-work"));
    ...
    fn validate_instance_accepts_relocated_root() {
        let a = adapter();
        let inst = sample_instance_with_root(&crate::test_util::tmp_abs_str(".mimo-work"));
    ...
    fn wrapper_env_var_isolation_is_relocated_root() {
        let tmp_root = crate::test_util::tmp_abs_str("user/.mimo-isolated");
        let a = adapter();
