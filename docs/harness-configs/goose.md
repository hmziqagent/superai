...
> **Live-verified layout (goose 1.51.0, 2026-09-18, area-5 evidence `.z-workflow/evidence/live/goose/`)** — `goose info` reports, with `GOOSE_PATH_ROOT` set: Config dir `$GOOSE_PATH_ROOT/config`, Config yaml `$GOOSE_PATH_ROOT/config/config.yaml`, Sessions DB (sqlite) `$GOOSE_PATH_ROOT/data/sessions/sessions.db`, Logs dir `$GOOSE_PATH_ROOT/state/logs`. Default (unset) layout under `$HOME`: Config dir `~/.config/goose` (+ `config.yaml`, `secrets.yaml`, `recipes/`, `skills/` inside), Sessions DB `~/.local/share/goose/sessions/sessions.db` (sqlite — **not** per-session `.jsonl` files, and **not** under `~/.config/goose/`), Logs `~/.local/state/goose/logs`. Decisive read-path proof: a provider config seeded at `$GOOSE_PATH_ROOT/config/config.yaml` is loaded (`goose run` banner `openai gpt-4o-mini`; `goose doctor` proceeds to the 401 auth wall), while the SAME file at the flat `$GOOSE_PATH_ROOT/config.yaml` yields `No provider configured. Run 'goose configure' first.` — config files live one level deeper than the env-var dir itself. User skills resolve under the config dir too: a SKILL.md at `$GOOSE_PATH_ROOT/config/skills/<author>/SKILL.md` is listed by `goose skills list`, a copy at `$GOOSE_PATH_ROOT/skills/` is ignored.
